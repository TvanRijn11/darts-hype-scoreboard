...
import { Player, GameOptions } from "@/src/types/game";
import { MiniCheckout } from "./CheckoutDisplay";
...
export const Scoreboard: React.FC<ScoreboardProps> = ({
  ...
  const showLegs = gameOptions && gameOptions.bestOfLegs > 1;
            ...
            <div className="p-4 md:p-6 flex flex-col items-center justify-center text-center">
              {showLegs && (
                <div className="text-xs font-bold text-zinc-500 mb-1">
                  {player.legsWon || 0} / {gameOptions.bestOfLegs}
                </div>
              )}
              <h2
              ...
              {isActive && player.score > 0 && player.score <= 170 && (
                <MiniCheckout score={player.score} />
              )}
            </div>
          </div>
        );
